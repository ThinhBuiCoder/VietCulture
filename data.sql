...
-- Thêm các cột Google OAuth vào bảng Users hiện tại
ALTER TABLE Users ADD 
    googleId NVARCHAR(255) NULL,
    provider NVARCHAR(20) NOT NULL DEFAULT 'local' CHECK (provider IN ('local', 'google', 'facebook', 'both'));
GO

-- Tạo index để tìm kiếm nhanh
CREATE INDEX IX_Users_GoogleId ON Users(googleId);
CREATE INDEX IX_Users_Provider ON Users(provider);
CREATE INDEX IX_Users_Email_Provider ON Users(email, provider);
GO
...
-- Tạo bảng lưu lịch sử đăng nhập OAuth (optional)
...
-- Index cho bảng OAuthLogins
CREATE INDEX IX_OAuthLogins_UserId ON OAuthLogins(userId);
CREATE INDEX IX_OAuthLogins_Provider ON OAuthLogins(provider);
CREATE INDEX IX_OAuthLogins_LoginTime ON OAuthLogins(loginTime);
GO

-- Test data: Tạo một user Google để test
INSERT INTO Users (email, password, fullName, userType, isActive, emailVerified, googleId, provider)
VALUES ('testgoogle@gmail.com', 'dummy_password_for_google', N'Test Google User', 'TRAVELER', 1, 1, '123456789012345678901', 'google');

DECLARE @testUserId INT = SCOPE_IDENTITY();
INSERT INTO Travelers (userId, preferences) VALUES (@testUserId, '{"likes": ["Food", "Culture"]}');
GO

PRINT 'Database updated successfully for Google OAuth!';
PRINT 'Added columns: googleId, provider to Users table';
PRINT 'Created OAuthLogins table for logging';
PRINT 'Added test Google user: testgoogle@gmail.com';
GO