...
public class BookingServlet extends HttpServlet {
    ...
    /**
     * Handle booking history page
     */
    private void handleBookingHistory(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException, SQLException {
        LOGGER.info("Handling booking history page");
        ...
        try {
            ...
            // Add booking details for each booking
            for (Booking booking : bookings) {
                if (booking.getExperienceId() != null) {
                    Experience experience = experienceDAO.getExperienceById(booking.getExperienceId());
                    if (experience != null) {
                        booking.setExperienceName(experience.getTitle());
                        ...
                        if (experience.getImages() != null && !experience.getImages().trim().isEmpty()) {
                            ...
                            if (imageArray.length > 0) {
                                booking.setServiceImage(imageArray[0].trim());
                            }
                        }
                    }
                    ...
                    if (accommodation != null) {
                        ...
                        // Get first image from images list
                        if (accommodation.getImages() != null && !accommodation.getImages().trim().isEmpty()) {
            ...
            // Forward to booking history page
            request.getRequestDispatcher("/view/jsp/home/booking-history.jsp").forward(request, response);
            
        } catch (SQLException e) {
            LOGGER.log(Level.SEVERE, "Error retrieving booking history", e);
            request.setAttribute("error", "Không thể tải lịch sử đặt chỗ. Vui lòng thử lại sau.");
            request.getRequestDispatcher("/view/jsp/home/booking-history.jsp").forward(request, response);
        }
    }
    ...
    private void handleCancelBooking(HttpServletRequest request, HttpServletResponse response, User user)
        ...
        try {
            ...
            if (booking.getTravelerId() != user.getUserId()) {
                sendErrorResponse(request, response, "You can only cancel your own bookings");
                return;
            }
            ...
            if (success) {
                LOGGER.info("Booking cancelled successfully: " + bookingId + " by user: " + user.getUserId());
                
                // Send JSON response
                response.setContentType("application/json");
                response.setCharacterEncoding("UTF-8");
                response.getWriter().write("{\"success\": true, \"message\": \"Đã hủy đặt chỗ thành công\"}");
            } else {
    ...
    private void sendErrorResponse(HttpServletRequest request, HttpServletResponse response, String message)
        ...
        if ("XMLHttpRequest".equals(requestedWith) || request.getParameter("ajax") != null) {
