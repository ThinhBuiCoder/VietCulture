...
@WebServlet(name = "ManageServicesServlet", urlPatterns = {
    ...
    "/host/services/promotion",    // Thêm pattern cho promotion
...
public class ManageServicesServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            else if ("/host/services/promotion".equals(servletPath)) {
                LOGGER.info("Handling promotion request");
                handlePromotion(request, response, currentUser);
            }
...
private void handleToggleService(HttpServletRequest request, HttpServletResponse response, User user, String pathInfo)
     ...
     */
    private void handlePromotion(HttpServletRequest request, HttpServletResponse response, User user)
            throws ServletException, IOException {
        LOGGER.info("=== DEBUG PROMOTION ===");
        ...
        try {
            ...
            // Validation
            if (serviceType == null || serviceIdStr == null || promotionPercentStr == null ||
                promotionStartStr == null || promotionEndStr == null) {
                LOGGER.warning("Missing required parameters for promotion");
                setErrorMessage(request, "Thiếu tham số cần thiết");
                response.sendRedirect(request.getContextPath() + "/host/services/manage");
                return;
            }

            LOGGER.info("Parameters: serviceType=" + serviceType + ", serviceId=" + serviceIdStr +
                       ", promotionPercent=" + promotionPercentStr + ", promotionStart=" + promotionStartStr +
                       ", promotionEnd=" + promotionEndStr);

            int serviceId;
            int promotionPercent;
            java.sql.Timestamp promotionStart;
            java.sql.Timestamp promotionEnd;

            try {
                serviceId = Integer.parseInt(serviceIdStr.trim());
                promotionPercent = Integer.parseInt(promotionPercentStr.trim());
                promotionStart = java.sql.Timestamp.valueOf(promotionStartStr.replace("T", " ") + ":00");
                promotionEnd = java.sql.Timestamp.valueOf(promotionEndStr.replace("T", " ") + ":00");
                
                // Sử dụng PromotionUtils để validate
                String validationError = PromotionUtils.validatePromotionData(promotionPercent, promotionStart, promotionEnd);
                ...
                setErrorMessage(request, "Dữ liệu không hợp lệ: " + e.getMessage());
                response.sendRedirect(request.getContextPath() + "/host/services/manage");
                return;
            }
            ...
            if ("experience".equals(serviceType)) {
                ...
                if (experience.getHostId() != user.getUserId()) {
                    LOGGER.warning("Access denied for experience " + serviceId + " by user " + user.getUserId());
                    setErrorMessage(request, "Bạn không có quyền cập nhật dịch vụ này");
                    response.sendRedirect(request.getContextPath() + "/host/services/manage");
                    return;
                }
                updateSuccess = experienceDAO.updatePromotion(serviceId, promotionPercent, promotionStart, promotionEnd);
                LOGGER.info("experienceDAO.updatePromotion result: " + updateSuccess);
            } else if ("accommodation".equals(serviceType)) {
                Accommodation accommodation = accommodationDAO.getAccommodationById(serviceId);
                if (accommodation == null) {
                    LOGGER.warning("Accommodation not found: " + serviceId);
                ...
                updateSuccess = accommodationDAO.updatePromotion(serviceId, promotionPercent, promotionStart, promotionEnd);
                LOGGER.info("accommodationDAO.updatePromotion result: " + updateSuccess);
            } else {
                LOGGER.warning("Invalid service type: " + serviceType);
                setErrorMessage(request, "Loại dịch vụ không hợp lệ: " + serviceType);
                response.sendRedirect(request.getContextPath() + "/host/services/manage");
            ...
            if (updateSuccess) {
                ...
                PromotionUtils.logPromotionActivity(serviceType, serviceId, promotionPercent, 
                                                   promotionStart, promotionEnd, "APPLIED");
                setSuccessMessage(request, "Áp dụng khuyến mãi thành công!");
                LOGGER.info("Promotion updated successfully for " + serviceType + " ID: " + serviceId);
            } else {
                setErrorMessage(request, "Không thể cập nhật khuyến mãi. Vui lòng kiểm tra lại.");
                LOGGER.warning("Failed to update promotion for " + serviceType + " ID: " + serviceId);
            }

            response.sendRedirect(request.getContextPath() + "/host/services/manage");

        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "Error in handlePromotion", e);
            setErrorMessage(request, "Lỗi khi xử lý khuyến mãi: " + e.getMessage());
            response.sendRedirect(request.getContextPath() + "/host/services/manage");
        }
    }
}