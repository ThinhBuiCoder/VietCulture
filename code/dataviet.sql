...
-- Add promotion fields to Experiences table if they don't exist
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Experiences' AND COLUMN_NAME = 'promotion_percent')
BEGIN
    ALTER TABLE Experiences 
    ADD promotion_percent INT NOT NULL DEFAULT 0,
        promotion_start DATETIME NULL,
        promotion_end DATETIME NULL;
    
    PRINT 'Promotion fields added successfully to Experiences table';
END
ELSE
BEGIN
    PRINT 'Promotion fields already exist in Experiences table';
END;

-- Add promotion fields to Accommodations table if they don't exist
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Accommodations' AND COLUMN_NAME = 'promotion_percent')
BEGIN
    ALTER TABLE Accommodations 
    ADD promotion_percent INT NOT NULL DEFAULT 0,
        promotion_start DATETIME NULL,
    ...
    PRINT 'Promotion fields added successfully to Accommodations table';
END
ELSE
BEGIN
    PRINT 'Promotion fields already exist in Accommodations table';
END;
